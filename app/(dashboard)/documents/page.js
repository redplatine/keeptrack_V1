...
export default function DocumentsPage() {
  ...
  const fetchEmployeDetails = useCallback(async (id) => {
    if (!id || !entrepriseId) return
    ...
    if (empError) {
      console.error('Erreur chargement employé document:', empError)
    ...
    if (solError) {
      console.error('Erreur chargement soldes document:', solError)
    }
    ...
    if (absError) {
      console.error('Erreur chargement absences document:', absError)
    }
  ...
  const fetchData = useCallback(async () => {
    ...
    if (empError) {
      console.error('Erreur récupération employé courant:', empError)
      setLoading(false)
      return
    }

    setCurrentRole(emp?.role || null)
    ...
    if (socError) {
      console.error('Erreur récupération société:', socError)
    }
    ...
    if (emp?.role === 'admin' || emp?.role === 'manager') {
      ...
      if (empsError) {
        console.error('Erreur récupération liste employés:', empsError)
        setEmployes([])
      } else {
        setEmployes(emps || [])
        if (emps?.length > 0) {
      ...
      setSelectedEmployeId(emp?.id || null)
      setSelectedEmploye(emp || null)
      if (emp?.id) {
        await fetchEmployeDetails(emp.id)
      }
    }

    setLoading(false)
  }, [entrepriseId, fetchEmployeDetails])
  ...
  useEffect(() => {
    if (selectedEmployeId && entrepriseId) {
      fetchEmployeDetails(selectedEmployeId)
    }
  }, [selectedEmployeId, entrepriseId, fetchEmployeDetails])
  ...
  const genererAttestation = async () => {
    ...
    try {
      ...
      const intro = doc.splitTextToSize(
        `Je soussigné(e), ${signataire}, ${qualite} de la société ${
          societe.raison_sociale || '[Société]'
        }, certifie que ${nomComplet} est employé(e) au sein de notre établissement dans les conditions suivantes :`,
        contentW
      )
      ...
      const fields = [
        ...
        ['Date d’entrée', selectedEmploye.date_entree ? formatDate(selectedEmploye.date_entree) : '—'],
      ...
      doc.text(
        `${societe.raison_sociale || ''} · ${adresseSociete} · SIRET ${societe.siret || ''}`,
        W / 2,
        290,
        { align: 'center' }
      )
      ...
      doc.save(
        `attestation_employeur_${selectedEmploye.nom}_${selectedEmploye.prenom}_${
  ...
  const genererRecapConges = async () => {
    ...
    try {
      ...
      doc.setTextColor(120, 113, 108)
      doc.text(
        `${selectedEmploye.poste || '—'} · ${selectedEmploye.type_contrat || '—'} · Entrée le ${
          selectedEmploye.date_entree ? formatDate(selectedEmploye.date_entree) : '—'
        }`,
        marginL + 8,
        y + 16
      )
      ...
      lignes.forEach((l, i) => {
        ...
        doc.setFontSize(8)
        doc.setTextColor(
          sv > 5 ? 22 : sv > 0 ? 180 : 220,
          sv > 5 ? 163 : sv > 0 ? 120 : 38,
          sv > 5 ? 74 : 38
        )
      ...
      if (absAnnee.length === 0) {
        ...
        absAnnee.forEach((a, i) => {
          ...
          doc.text(formatDate(a.date_debut), marginL + 55, y + 6)
          doc.text(formatDate(a.date_fin), marginL + 90, y + 6)
          doc.setFont('helvetica', 'bold')
      ...
      doc.setFontSize(8)
      doc.setTextColor(168, 162, 158)
      doc.text(`${societe?.raison_sociale || ''} · SIRET ${societe?.siret || ''}`, W / 2, 290, {
        align: 'center',
      })
      doc.text(`Document généré le ${today} via KeepTrack`, W / 2, 295, { align: 'center' })
      ...
      console.error(error)
      alert('Erreur lors de la génération du récapitulatif.')
    } finally {
      setGenerating(null)
    }
  }
    ...
    <div style={{ padding: '24px 40px 40px', fontFamily: "'Inter', -apple-system, sans-serif", minHeight: '100vh' }}>
      ...
      {selectedEmploye && (
        ...
        >
          ...
          >
            {selectedEmploye.prenom?.[0]}
            {selectedEmploye.nom?.[0]}
          </div>

          <div style={{ flex: 1 }}>
            <p style={{ fontSize: '15px', fontWeight: 700, color: '#1C1917', margin: '0 0 3px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
        ...
        >
          ...
          >
            ...
            <div>
              <h3 style={{ fontSize: '14px', fontWeight: 700, color: '#1C1917', margin: '0 0 3px' }}>
                Attestation employeur
              </h3>
              <p style={{ fontSize: '12px', color: '#A8A29E', margin: 0 }}>
                Certifie qu'un salarié est bien employé dans la société
              </p>
            </div>
          </div>
          ...
          <div style={{ padding: '16px 24px' }}>
            ...
            {!societe?.nom_signataire && (
              ...
              >
                ...
                <span style={{ fontSize: '11.5px', color: '#B45309' }}>
                  Renseignez le signataire dans l'onglet Société
                </span>
              </div>
            )}
            ...
            <button
              ...
              disabled={generating === 'attestation' || !selectedEmploye}
              style={{
                width: '100%',
                padding: '10px',
                borderRadius: '10px',
              ...
              onMouseEnter={(e) => {
                if (generating !== 'attestation') e.currentTarget.style.opacity = '0.9'
              }}
              onMouseLeave={(e) => {
                if (generating !== 'attestation') e.currentTarget.style.opacity = '1'
              }}
            >
              {generating === 'attestation' ? (
                <>
        ...
        >
          ...
          >
            ...
            <div>
              ...
              <p style={{ fontSize: '12px', color: '#A8A29E', margin: 0 }}>
                Tableau de bord complet des congés de l'année en cours
              </p>
            </div>
          </div>
          ...
          <div style={{ padding: '16px 24px' }}>
            ...
            <button
              onClick={genererRecapConges}
              disabled={generating === 'recap' || !selectedEmploye}
              style={{
                width: '100%',
                padding: '10px',
                borderRadius: '10px',
                ...
                background:
                  generating === 'recap'
                    ? '#C4B5A5'
                    : 'linear-gradient(135deg, #1D4ED8, #4F7EF7)',
                color: 'white',
                fontSize: '13.5px',
                fontWeight: 600,
              ...
              onMouseEnter={(e) => {
                if (generating !== 'recap') e.currentTarget.style.opacity = '0.9'
              }}
              onMouseLeave={(e) => {
                if (generating !== 'recap') e.currentTarget.style.opacity = '1'
              }}
            >
              {generating === 'recap' ? (
                <>
      ...
      >
        <p style={{ fontSize: '12px', color: '#78716C', margin: 0 }}>
          💡 Les documents PDF sont générés directement dans votre navigateur. Les informations
          proviennent des données renseignées dans KeepTrack. Pensez à renseigner le signataire
          dans l'onglet <strong>Société</strong>.
        </p>
      </div>
    </div>
  )
