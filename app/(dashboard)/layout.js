...
const PAGE_META = {
  ...
  '/documents': { title: 'Documents', sub: () => 'Générez et consultez vos documents' },
...
export default function DashboardLayout({ children }) {
  ...
  useEffect(() => {
    if (!entrepriseId || !role) return

    const channels = []

    if (role === 'admin' || role === 'manager') {
      const absChannel = supabase
        .channel(`absences-notifs-${entrepriseId}`)
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'absences' },
          () => fetchAbsencesEnAttente(entrepriseId)
        )
        .subscribe()

      channels.push(absChannel)
    }

    if (role === 'admin') {
      const msgChannel = supabase
        .channel(`messages-notifs-${entrepriseId}`)
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'messages' },
          () => fetchMessagesOuverts(entrepriseId)
        )
        .subscribe()

      const recupChannel = supabase
        .channel(`recup-notifs-${entrepriseId}`)
        .on(
          'postgres_changes',
          { event: '*', schema: 'public', table: 'demandes_recup' },
          () => fetchDemandesRecup(entrepriseId)
        )
        .subscribe()

      channels.push(msgChannel, recupChannel)
    }
  ...
  const fetchAbsencesEnAttente = async (entrepriseIdParam) => {
    if (!entrepriseIdParam) return

    const { count, error } = await supabase
      .from('absences')
      .select('*', { count: 'exact', head: true })
      .eq('entreprise_id', entrepriseIdParam)
      .eq('statut', 'En attente')

    if (error) {
      console.error('Erreur fetchAbsencesEnAttente:', error)
      return
    }
  ...
  const fetchMessagesOuverts = async (entrepriseIdParam) => {
    if (!entrepriseIdParam) return
    ...
    if (error) {
      console.error('Erreur fetchMessagesOuverts:', error)
      return
    }
  ...
  const fetchDemandesRecup = async (entrepriseIdParam) => {
    if (!entrepriseIdParam) return
    ...
    if (employesError) {
      console.error('Erreur récupération employés pour recup:', employesError)
      return
    }

    const employeIds = (employesData || []).map((e) => e.id)
    ...
    if (error) {
      console.error('Erreur fetchDemandesRecup:', error)
      return
    }
  ...
  return (
    <div className="min-h-screen flex" style={{ fontFamily: "'Inter', -apple-system, sans-serif" }}>
        ...
        <div className="px-5 pt-7 pb-5" style={{ borderBottom: `1px solid ${SIDEBAR.border}` }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h1 className="font-bold leading-tight" style={{ fontSize: '15px', color: '#F0E8EA', letterSpacing: '-0.3px' }}>
                  KeepTrack
                </h1>
                    ...
                    {role === 'admin' && (
                      <Link href="/compteurs" onClick={() => setClochePaneau(false)} style={{ textDecoration: 'none' }}>
                        ...
                        >
                          ...
                          <span
                            style={{
                              background: demandesRecup > 0 ? '#FFFBEB' : '#F0EDE9',
                              color: demandesRecup > 0 ? '#B45309' : '#A8A29E',
                              fontSize: '12px',
                              fontWeight: 700,
                              padding: '2px 8px',
                              borderRadius: '20px',
      ...
      <div
        ...
        style={{ display: 'flex', flexDirection: 'column', position: 'relative', background: '#F7F5F3' }}
      >
        <svg
          style={{
            position: 'fixed',
