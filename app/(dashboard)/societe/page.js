...
export default function SocietePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (empError) {
      console.error('Erreur récupération rôle:', empError)
      setRole(null)
    } else {
  ...
  const handleSave = async (e) => {
    ...
    if (societe) {
      ...
      if (error) {
        alert('Erreur mise à jour société : ' + error.message)
        return
      }
      ...
      const { data, error } = await supabase
        .from('societe')
        .insert([{ ...form, entreprise_id: entrepriseId }])
        .select()
        .single()

      if (error) {
        alert('Erreur création société : ' + error.message)
        return
      }
            ...
            >
              ...
              <div>
                <h2 style={{ fontSize: '17px', fontWeight: 700, color: '#1C1917', margin: 0 }}>
                  {societe?.raison_sociale || 'Raison sociale non renseignée'}
                </h2>
                <p style={{ fontSize: '13px', color: '#A8A29E', margin: '2px 0 0' }}>
            ...
            <div style={{ display: 'grid', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, nom_signataire: e.target.value })}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setForm({ ...form, qualite_signataire: e.target.value })}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '12px' }}>
                <div>
                  ...
                  <input
                    value={form.numero_voie}
                    onChange={(e) => setForm({ ...form, numero_voie: e.target.value })}
                    placeholder="12"
                    style={S.input}
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    value={form.nom_rue}
                    onChange={(e) => setForm({ ...form, nom_rue: e.target.value })}
