...
function Avatar({ prenom, nom }) {
  ...
  return (
    <div style={{ width: '36px', height: '36px', borderRadius: '10px', flexShrink: 0, background: '#F2E6E9', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', fontWeight: 700, color: '#6B2F42' }}>
      {initiales || '?'}
    </div>
  )
}
...
export default function MessagesPage() {
  ...
  useEffect(() => { if (entrepriseId) fetchMessages() }, [entrepriseId])
  ...
  return (
    <div style={{ padding: '0 40px 40px', fontFamily: "'Inter', -apple-system, sans-serif", minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <select value={filtreStatut} onChange={e => setFiltreStatut(e.target.value)} style={{ border: '1px solid #E8E4E0', borderRadius: '10px', padding: '9px 12px', fontSize: '13.5px', background: 'white', outline: 'none', color: '#44403C', fontFamily: 'inherit', cursor: 'pointer' }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {messagesFiltres.map(msg => {
            ...
            return (
              <div key={msg.id} style={{ background: 'white', borderRadius: '16px', border: `1px solid ${isOpen ? '#FDE68A' : '#E8E4E0'}`, boxShadow: isOpen ? '0 2px 12px rgba(245,158,11,0.08)' : '0 1px 4px rgba(0,0,0,0.04)', overflow: 'hidden' }}>

                <div onClick={() => toggleExpand(msg.id)}
                  style={{ padding: '18px 24px', display: 'flex', alignItems: 'center', gap: '14px', cursor: 'pointer', borderBottom: isExpanded ? '1px solid #F0EDE9' : 'none' }}
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '3px' }}>
                      ...
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', fontSize: '11px', fontWeight: 600, padding: '3px 8px', borderRadius: '20px', background: sc.bg, color: sc.color, border: `1px solid ${sc.border}`, flexShrink: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }} onClick={e => e.stopPropagation()}>
                    {isOpen && (
                      <button onClick={() => handleCloture(msg.id)} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '5px 12px', borderRadius: '8px', border: '1px solid #BBF7D0', background: '#F0FDF4', color: '#16A34A', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                    ...
                    <button onClick={() => handleSupprimer(msg.id)} style={{ display: 'flex', alignItems: 'center', gap: '5px', padding: '5px 12px', borderRadius: '8px', border: '1px solid #E8E4E0', background: '#FAF8F6', color: '#A8A29E', fontSize: '12px', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                ...
                {isExpanded && (
                  <div style={{ padding: '20px 24px' }}>
                    <div style={{ marginBottom: '16px' }}>
                      <p style={{ fontSize: '11px', fontWeight: 600, color: '#A8A29E', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 8px' }}>Message</p>
                    ...
                    {msg.reponse_admin ? (
                      <div style={{ marginBottom: '16px' }}>
                        <p style={{ fontSize: '11px', fontWeight: 600, color: '#A8A29E', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 8px' }}>Votre réponse</p>
                      ...
                      <div>
                        <p style={{ fontSize: '11px', fontWeight: 600, color: '#A8A29E', textTransform: 'uppercase', letterSpacing: '0.07em', margin: '0 0 8px' }}>Répondre</p>
                        <textarea
                          value={reponses[msg.id] || ''}
                          onChange={e => setReponses(prev => ({ ...prev, [msg.id]: e.target.value }))}
                          rows={3} placeholder="Rédigez votre réponse…"
                          style={{ width: '100%', border: '1px solid #E8E4E0', borderRadius: '12px', padding: '12px 14px', fontSize: '13.5px', background: '#FAF8F6', outline: 'none', color: '#1C1917', fontFamily: 'inherit', resize: 'vertical', marginBottom: '10px', boxSizing: 'border-box', lineHeight: 1.6 }}
                          onFocus={e => e.target.style.borderColor = '#8B4A5A'}
                          onBlur={e => e.target.style.borderColor = '#E8E4E0'}
                        />
                        <button onClick={() => handleRepondre(msg)} disabled={!reponses[msg.id]?.trim()}
                          style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: '10px', border: 'none', background: reponses[msg.id]?.trim() ? '#1C1917' : '#E8E4E0', color: reponses[msg.id]?.trim() ? 'white' : '#A8A29E', fontSize: '13.5px', fontWeight: 500, cursor: reponses[msg.id]?.trim() ? 'pointer' : 'not-allowed', fontFamily: 'inherit', transition: 'background 0.15s' }}>
