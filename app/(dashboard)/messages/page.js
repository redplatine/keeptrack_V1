...
export default function MessagesPage() {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    if (employesError) {
      console.error('Erreur chargement employés messages:', employesError)
    }
  ...
  useEffect(() => {
    if (entrepriseId) fetchMessages()
  }, [entrepriseId, fetchMessages])

  const handleRepondre = async (msg) => {
    const reponse = reponses[msg.id]?.trim()

    if (!reponse || !entrepriseId) return
    ...
    if (updateError) {
      alert('Erreur réponse : ' + updateError.message)
      return
    }
    ...
    >
      ...
      >
        <p style={{ fontSize: '13px', color: '#78716C', margin: 0 }}>
          {nbOuverts > 0 ? (
            <>
              <span style={{ color: '#B45309', fontWeight: 600 }}>{nbOuverts}</span> message(s)
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <p style={{ fontSize: '12px', color: '#A8A29E', margin: 0 }}>
                      <span style={{ color: '#78716C', fontWeight: 500 }}>
                        {msg.employe?.prenom} {msg.employe?.nom}
                      </span>
                      {' · '}
