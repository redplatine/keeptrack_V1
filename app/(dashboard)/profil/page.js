    ...
    >
      ...
      <div style={{ padding: '16px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: '4px', marginBottom: '16px' }}>
          <span style={{ fontSize: '36px', fontWeight: 700, color: '#1C1917', lineHeight: 1 }}>
            {solde ?? '—'}
          </span>
...
export default function ProfilPage() {
  ...
  const getAvatarPublicUrl = (employeId) => {
    const { data } = supabase.storage.from('avatars').getPublicUrl(`${employeId}/avatar`)
  ...
  const fetchProfil = useCallback(async () => {
    ...
    if (empError) {
      console.error('Erreur récupération profil employé:', empError)
      setEmploye(null)
      setSoldes(null)
      setLoading(false)
      return
    ...
    if (emp) {
      ...
      if (soldesError) {
        console.error('Erreur récupération soldes:', soldesError)
        setSoldes(null)
      } else {
        setSoldes(soldesData || null)
      }

      const publicUrl = getAvatarPublicUrl(emp.id)
      setAvatar(publicUrl ? `${publicUrl}?t=${Date.now()}` : null)
  ...
  useEffect(() => {
    if (entrepriseId) fetchProfil()
  }, [entrepriseId, fetchProfil])

  const handleAvatarUpload = async (e) => {
    const file = e.target.files?.[0]
    if (!file || !employe?.id) return
    ...
    if (error) {
      ...
      const publicUrl = getAvatarPublicUrl(employe.id)
      setAvatar(publicUrl ? `${publicUrl}?t=${Date.now()}` : null)
      setAvatarError(false)
    }
  ...
  const salaire = employe.salaire_brut
    ? new Intl.NumberFormat('fr-FR', {
        style: 'currency',
        currency: 'EUR',
        maximumFractionDigits: 0,
      }).format(employe.salaire_brut)
    : null
    ...
    >
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: '14px', color: '#78716C', margin: '0 0 2px' }}>
            {employe.poste || '—'}
          </p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px', marginBottom: '20px' }}>
        ...
        >
          ...
          <InfoField label="Date de naissance" value={formatDate(employe.date_naissance)} />
        ...
        >
          ...
          <InfoField label="Date d'entrée" value={formatDate(employe.date_entree)} />
          ...
          {employe.forfait_jours ? (
            <InfoField
              label="Jours annuels"
              value={employe.nb_jours_annuels ? `${employe.nb_jours_annuels} jours` : null}
            />
          ) : (
            <InfoField
              label="Heures par semaine"
              value={employe.nb_heures_semaine ? `${employe.nb_heures_semaine}h / semaine` : null}
            />
          )}
          <InfoField
            label="RTT annuel"
            value={employe.rtt_annuel ? `${employe.rtt_annuel} jours` : null}
          />
