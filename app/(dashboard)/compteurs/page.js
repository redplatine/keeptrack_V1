...
// Solde calculé automatiquement — non éditable
...
// Ligne employé dans le tableau compteurs
    ...
    <div style={{ background: 'white', borderRadius: 14, border: '1px solid #E8E4E0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', padding: '12px 16px', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 90px', gap: 8, alignItems: 'center', transition: 'box-shadow 0.15s' }}
      ...
      {GROUPES.map(g => (
        <div key={g.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
          ...
          {/* Barre de progression sous le groupe */}
          {(s[`${g.key}_acquis`] || 0) > 0 && (
      ...
      {/* Récup — solde éditable directement */}
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
...
export default function CompteursPage() {
  ...
  const handleDelta = async (employeId, field, delta) => {
    ...
    if (solde) await supabase.from('soldes_conges').update(updates).eq('id', solde.id)
    else await supabase.from('soldes_conges').insert({ employe_id: employeId, annee, ...updates })
    await fetchAll()
  }
  ...
  const handleValiderDemande = async (demande) => {
    ...
    if (solde) {
      ...
      await supabase.from('soldes_conges').insert({ employe_id: demande.employe_id, annee, recup_acquis: demande.nb_jours, recup_solde: demande.nb_jours })
    }
