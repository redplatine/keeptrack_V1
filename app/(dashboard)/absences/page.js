...
const joursFeries = [
  ...
  '2027-01-01','2027-03-29','2027-05-01','2027-05-08','2027-05-17','2027-05-28','2027-07-14','2027-08-15','2027-11-01','2027-11-11','2027-12-25',
...
function calculJoursOuvres(debut, fin) {
  let count = 0, current = new Date(debut)
  const end = new Date(fin)
  while (current <= end) {
    const day = current.getDay(), dateStr = current.toISOString().split('T')[0]
...
function calculJoursCalendaires(debut, fin) {
  return Math.round((new Date(fin) - new Date(debut)) / (1000 * 60 * 60 * 24)) + 1
}
function calculNbJours(typeAbsence, debut, fin, demiJournee) {
  if (demiJournee) return 0.5
  if (!debut || !fin) return 0
  return ABSENCES_CALENDAIRE.includes(typeAbsence) ? calculJoursCalendaires(debut, fin) : calculJoursOuvres(debut, fin)
}
function genererOptionsMois() {
  const options = [], now = new Date()
  for (let i = -12; i <= 12; i++) {
    const d = new Date(now.getFullYear(), now.getMonth() + i, 1)
    options.push({ value: `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`, label: `${MOIS[d.getMonth()]} ${d.getFullYear()}` })
  }
  return options
}

const S = {
  input: { width: '100%', border: '1px solid #E8E4E0', borderRadius: '10px', padding: '9px 12px', fontSize: '13.5px', background: '#FAF8F6', outline: 'none', color: '#1C1917', fontFamily: 'inherit' },
  select: { border: '1px solid #E8E4E0', borderRadius: '10px', padding: '9px 12px', fontSize: '13.5px', background: '#FAF8F6', outline: 'none', color: '#1C1917', fontFamily: 'inherit', cursor: 'pointer' },
...
export default function AbsencesPage() {
  ...
  const [form, setForm] = useState({ employe_id: '', type_absence: 'CP', date_debut: '', date_fin: '', demi_journee: false, commentaire_salarie: '' })
  ...
  const absencesFiltrees = useMemo(() => absences.filter(abs => {
    if (filtreMois) {
      const d = abs.date_debut?.slice(0, 7), f = abs.date_fin?.slice(0, 7)
      if (d > filtreMois || f < filtreMois) return false
    }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault(); setAlerteForm(null)
  ...
  const handleDemandeRecup = async (e) => {
    e.preventDefault(); setAlerteRecup(null)
    const nb = parseFloat(formRecup.nb_jours)
    if (!nb || nb <= 0) { setAlerteRecup('Nombre de jours invalide.'); return }
    const { error } = await supabase.from('demandes_recup').insert([{ employe_id: currentEmploye.id, nb_jours: nb, motif: formRecup.motif || null }])
    if (error) { setAlerteRecup('Erreur : ' + error.message); return }
    setShowFormRecup(false); setFormRecup({ nb_jours: '', motif: '' }); setSuccessRecup(true)
  ...
  const handleExportExcel = () => {
    const data = absencesFiltrees.map(abs => ({
      'Matricule': abs.employes?.matricule || '—', 'Employé': `${abs.employes?.prenom||''} ${abs.employes?.nom||''}`,
      "Type d'absence": abs.type_absence, 'Date début': abs.date_debut, 'Date fin': abs.date_fin || abs.date_debut,
      'Nombre de jours': abs.nb_jours, 'Statut': abs.statut, 'Commentaire': abs.commentaire_salarie || '',
    }))
    ...
    <div style={{ padding: '0 40px 40px', fontFamily: "'Inter', -apple-system, sans-serif", minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          {!isManager && (
            <button onClick={() => { setShowFormRecup(!showFormRecup); setShowForm(false); setAlerteRecup(null) }} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: '10px', border: '1px solid #FDE68A', background: showFormRecup ? '#FEF3C7' : '#FFFBEB', color: '#B45309', fontSize: '13.5px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}
          ...
          <button onClick={() => { setShowForm(!showForm); setShowFormRecup(false); setAlerteForm(null) }} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '9px 16px', borderRadius: '10px', border: 'none', background: '#1C1917', color: 'white', fontSize: '13.5px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}
      ...
      {showFormRecup && !isManager && (
        <div style={{ background: 'white', borderRadius: '14px', padding: '24px 28px', border: '1px solid #FDE68A', boxShadow: '0 2px 8px rgba(245,158,11,0.08)', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#A8A29E', margin: '0 0 20px' }}>Votre administrateur validera la demande et créditera votre compteur Récup.</p>
          {alerteRecup && <div style={{ background: '#FEF2F2', border: '1px solid #FECACA', borderRadius: '10px', padding: '12px 16px', marginBottom: '16px', color: '#DC2626', fontSize: '13.5px' }}>{alerteRecup}</div>}
          <form onSubmit={handleDemandeRecup}>
            <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: '16px', alignItems: 'end' }}>
              <div><label style={S.label}>Nombre de jours</label><input required type="number" step="0.5" min="0.5" value={formRecup.nb_jours} onChange={e => setFormRecup({ ...formRecup, nb_jours: e.target.value })} placeholder="Ex: 1, 0.5…" style={S.input} /></div>
              <div><label style={S.label}>Motif (optionnel)</label><input type="text" value={formRecup.motif} onChange={e => setFormRecup({ ...formRecup, motif: e.target.value })} placeholder="Ex: Heures supplémentaires semaine 12…" style={S.input} /></div>
              <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '10px' }}>
                <button type="submit" style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', background: '#B45309', color: 'white', fontSize: '13.5px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Envoyer la demande</button>
                <button type="button" onClick={() => { setShowFormRecup(false); setAlerteRecup(null) }} style={{ padding: '10px 20px', borderRadius: '10px', border: '1px solid #E8E4E0', background: 'white', color: '#78716C', fontSize: '13.5px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Annuler</button>
              </div>
            </div>
          </form>
        </div>
      ...
      <div style={{ background: 'white', borderRadius: '14px', padding: '18px 22px', border: '1px solid #E8E4E0', boxShadow: '0 1px 4px rgba(0,0,0,0.03)', marginBottom: '20px' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '16px', alignItems: 'flex-end' }}>
          <div><label style={S.label}>Mois</label>
          ...
          {isManager && (
            <div><label style={S.label}>Employé</label>
          ...
          {nbFiltresActifs > 0 && (
            <button onClick={() => { setFiltreMois(''); setFiltreStatut(''); setFiltreEmployeId('') }} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '9px 14px', borderRadius: '10px', border: '1px solid #E8E4E0', background: '#FAF8F6', color: '#78716C', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>
          ...
          <form onSubmit={handleSubmit}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginTop: '22px' }}>
                <input type="checkbox" id="demi_journee" checked={form.demi_journee} onChange={e => setForm({...form, demi_journee: e.target.checked, date_fin: ''})} style={{ width: '16px', height: '16px', accentColor: '#8B4A5A', cursor: 'pointer' }} />
              ...
              {(form.date_debut && (form.date_fin || form.demi_journee)) && (
                <div style={{ gridColumn: '1 / -1', background: '#F0EDE9', borderRadius: '12px', padding: '14px 18px', display: 'flex', alignItems: 'center', gap: '14px', border: '1px solid #E8E4E0' }}>
                  <div style={{ width: '44px', height: '44px', borderRadius: '12px', background: '#1C1917', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 700, color: 'white', flexShrink: 0 }}>{nbJoursCalcule}</div>
                  <div>
                    <p style={{ fontSize: '14px', fontWeight: 600, color: '#1C1917', margin: 0 }}>{nbJoursCalcule} jour(s) {form.demi_journee ? '' : ABSENCES_CALENDAIRE.includes(form.type_absence) ? 'calendaires' : 'ouvrés'}</p>
                    {ABSENCES_AUTO_APPROUVEES.includes(form.type_absence) && <span style={{ fontSize: '12px', background: '#F0FDF4', color: '#16A34A', padding: '2px 8px', borderRadius: '20px', fontWeight: 500 }}>✓ Approuvée automatiquement</span>}
                  </div>
                </div>
              )}
              <div style={{ gridColumn: '1 / -1' }}>
                <label style={S.label}>Commentaire (optionnel)</label>
                <textarea value={form.commentaire_salarie} onChange={e => setForm({...form, commentaire_salarie: e.target.value})} rows={2} placeholder="Précisez si nécessaire…" style={{ ...S.input, resize: 'none', lineHeight: 1.5 }} />
              </div>
              <div style={{ gridColumn: '1 / -1', display: 'flex', gap: '10px' }}>
                <button type="submit" style={{ padding: '10px 20px', borderRadius: '10px', border: 'none', background: '#1C1917', color: 'white', fontSize: '13.5px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Envoyer la demande</button>
                <button type="button" onClick={() => { setShowForm(false); setAlerteForm(null) }} style={{ padding: '10px 20px', borderRadius: '10px', border: '1px solid #E8E4E0', background: 'white', color: '#78716C', fontSize: '13.5px', fontWeight: 500, cursor: 'pointer', fontFamily: 'inherit' }}>Annuler</button>
              </div>
            </div>
          </form>
        </div>
      ...
      {loading ? (
        ...
        <div style={{ textAlign: 'center', padding: '60px', background: 'white', borderRadius: 16, border: '1px solid #E8E4E0' }}>
          ...
          <p style={{ color: '#A8A29E', fontSize: 14 }}>Aucune absence pour cette période</p>
        </div>
      ) : (
        <>
                ...
                <div key={abs.id} style={{ background: 'white', borderRadius: 14, border: '1px solid #E8E4E0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', padding: '13px 16px', display: 'grid', gridTemplateColumns: isManager ? '1fr 120px 180px 70px 130px 1fr' : '120px 180px 70px 130px 1fr', gap: 8, alignItems: 'center', transition: 'box-shadow 0.15s' }}
                  ...
                  {isManager && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 30, height: 30, borderRadius: 8, background: 'linear-gradient(135deg, #F2E6E9, #E8D5D9)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 800, color: '#6B2F42', flexShrink: 0 }}>
                        {abs.employes?.prenom?.[0]}{abs.employes?.nom?.[0]}
                      </div>
                      <div>
                        <p style={{ fontSize: 13, fontWeight: 600, color: '#1C1917', margin: 0 }}>{abs.employes?.prenom} {abs.employes?.nom}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                    {isManager && abs.statut === 'En attente' && (
                      <>
                        <button onClick={() => handleValidation(abs.id, 'Approuvée')} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, border: '1px solid #BBF7D0', background: '#F0FDF4', color: '#16A34A', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                          onMouseEnter={e => e.currentTarget.style.background = '#DCFCE7'}
                          onMouseLeave={e => e.currentTarget.style.background = '#F0FDF4'}>
                          <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
                          Approuver
                        </button>
                        <button onClick={() => handleValidation(abs.id, 'Refusée')} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, border: '1px solid #FECACA', background: '#FEF2F2', color: '#DC2626', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
                    ...
                    <button onClick={() => handleSupprimer(abs)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '5px 10px', borderRadius: 8, border: '1px solid #E8E4E0', background: '#FAF8F6', color: '#A8A29E', fontSize: 11, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' }}
