...
export default function ContactPage() {
  ...
  const fetchData = async () => {
    ...
    setEmploye(emp)
    if (emp) {
      const { data } = await supabase.from('messages').select('*').eq('employe_id', emp.id).order('created_at', { ascending: false })
      setMessages(data || [])
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    const { error } = await supabase.from('messages').insert([{ employe_id: employe.id, sujet: form.sujet, message: form.message }])
  ...
  return (
    <div style={{ padding: '36px 40px', fontFamily: "'Inter', -apple-system, sans-serif", background: '#F7F5F3', minHeight: '100vh', maxWidth: '760px' }}>
      ...
      {showForm && (
        <div style={{
          ...
          border: '1px solid #E8E4E0', boxShadow: '0 1px 4px rgba(0,0,0,0.04)', marginBottom: '20px'
        }}>
          ...
          {messages.map(msg => {
            ...
            return (
              <div key={msg.id} style={{
                ...
                border: `1px solid ${msg.statut === 'Ouvert' && !msg.reponse_admin ? '#FDE68A' : '#E8E4E0'}`,
                ...
                <div style={{ padding: '16px 22px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  ...
                  {msg.reponse_admin ? (
                    ...
                    <div style={{
                      ...
                      <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: '#F59E0B', flexShrink: 0, animation: 'pulse 2s infinite' }} />
                      <p style={{ fontSize: '12px', color: '#B45309', fontWeight: 500, margin: 0 }}>
