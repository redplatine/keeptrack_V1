...
export default function ContactPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    if (userError || !user?.email) {
      console.error('Erreur auth contact:', userError)
      setEmploye(null)
      setMessages([])
      return
    }
    ...
    if (empError) {
      console.error('Erreur récupération employé contact:', empError)
      setEmploye(null)
      setMessages([])
      return
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!entrepriseId || !employe?.id) {
      setErreur("Impossible d'envoyer le message pour le moment.")
      return
    }
      ...
      <div style={{ maxWidth: '760px' }}>
        ...
        >
          <p style={{ fontSize: '13px', color: '#78716C', margin: 0 }}>
            {nbOuverts > 0
              ? `${nbOuverts} message(s) en attente de réponse`
              : 'Échangez avec votre service RH'}
          </p>
        ...
        {messages.length === 0 ? (
          ...
          >
            ...
            <p style={{ color: '#C4B5A5', fontSize: '13px', margin: '4px 0 0' }}>
              Envoyez votre premier message au service RH
            </p>
          </div>
        ) : (
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: '16px',
                    border: `1px solid ${
                      msg.statut === 'Ouvert' && !msg.reponse_admin ? '#FDE68A' : '#E8E4E0'
                    }`,
                    boxShadow: '0 1px 4px rgba(0,0,0,0.04)',
                    overflow: 'hidden',
                  }}
