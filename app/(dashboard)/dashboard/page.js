    ...
    >
      ...
      <div style={{ padding: '22px 22px 18px 26px' }}>
        ...
        <p style={{ fontSize: 46, fontWeight: 800, color: '#1C1917', margin: '0 0 4px', lineHeight: 1 }}>
          {value ?? '—'}
          <span style={{ fontSize: 18, color: '#C4B5A5', fontWeight: 400, marginLeft: 5 }}>j</span>
        </p>
...
function LigneSalarie({ emp }) {
  ...
  const compteurs = [
    {
      label: 'SOLDE CP N-1',
      s: emp.solde?.cp_n1_solde,
      a: emp.solde?.cp_n1_acquis,
      p: emp.solde?.cp_n1_pris,
      color: '#8B4A5A',
      bg: '#F9EEF1',
    },
    {
      label: 'SOLDE CP N',
      s: emp.solde?.cp_n_solde,
      a: emp.solde?.cp_n_acquis,
      p: emp.solde?.cp_n_pris,
      color: '#4F7EF7',
      bg: '#EFF6FF',
    },
    {
      label: 'SOLDE RTT',
      s: emp.solde?.rtt_solde,
      a: emp.solde?.rtt_acquis,
      p: emp.solde?.rtt_pris,
      color: '#16A34A',
      bg: '#F0FDF4',
    },
    {
      label: 'SOLDE Récup',
      s: emp.solde?.recup_solde,
      a: emp.solde?.recup_acquis,
      p: emp.solde?.recup_pris,
      color: '#B45309',
      bg: '#FFFBEB',
    },
  ]
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          display: 'grid',
          gridTemplateColumns: '1fr 110px 110px 110px 110px 110px 28px',
          gap: 8,
          padding: '13px 16px',
          alignItems: 'center',
          cursor: 'pointer',
              ...
              <div style={{ width: 40, height: 3, background: '#F0EDE9', borderRadius: 99, overflow: 'hidden' }}>
                <div
                  style={{
                    ...
                    background: x.color,
                    borderRadius: 99,
                    width: `${Math.min(100, ((x.p || 0) / x.a) * 100)}%`,
                  }}
                />
              </div>
            )}
          ...
          {compteurs.map((x, i) => (
            <div
              ...
              style={{
                padding: '14px 16px',
                background: x.bg,
                borderRight: i < 3 ? '1px solid rgba(255,255,255,0.7)' : 'none',
              }}
            >
              <p
                style={{
...
export default function DashboardPage() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    if (userError || !user?.email) {
      console.error('Erreur auth dashboard:', userError)
      setEmploye(null)
      setSoldes(null)
      setEquipe([])
      setLoading(false)
    ...
    if (empError) {
      console.error('Erreur récupération employé dashboard:', empError)
      setEmploye(null)
      setSoldes(null)
      setEquipe([])
      setLoading(false)
    ...
    if (emp) {
      ...
      if (isManager) {
        ...
        if (employesError) {
          console.error('Erreur récupération équipe dashboard:', employesError)
          setEquipe([])
          setLoading(false)
          return
        }

        const { data: tousLesSoldes, error: soldesError } = await supabase
          .from('soldes_conges')
          .select('*')
          .eq('entreprise_id', entrepriseId)
          .eq('annee', annee)
        ...
        if (soldesError) {
          console.error('Erreur récupération soldes équipe:', soldesError)
        }

        const { data: absEnAttente, error: absError } = await supabase
          .from('absences')
          .select('employe_id')
          .eq('entreprise_id', entrepriseId)
        ...
        if (absError) {
          console.error('Erreur récupération absences en attente:', absError)
        }

        const equipeData = (employesData || [])
          .map((e) => ({
            ...e,
            solde: tousLesSoldes?.find((s) => s.employe_id === e.id) || null,
        ...
        const { data: soldesData, error: soldesError } = await supabase
          .from('soldes_conges')
          .select('*')
        ...
        if (soldesError) {
          console.error('Erreur récupération soldes salarié:', soldesError)
        }

        setSoldes(soldesData || null)
        setEquipe([])
      }
    } else {
      setSoldes(null)
      ...
      {isManager && (
        <div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: 20 }}>
            {[
              ...
              },
              {
                ...
                value: equipe.reduce((a, e) => a + e.demandesEnAttente, 0),
                color: '#B45309',
                bg: '#FFFBEB',
                icon: '⏳',
              },
              ...
              >
                ...
                <div>
                  ...
                  <p style={{ fontSize: 28, fontWeight: 800, color: '#1C1917', margin: 0, lineHeight: 1 }}>
                    {s.value}
                  </p>
                </div>
              </div>
            ...
            {['Solde CP N-1', 'Solde CP N', 'Solde RTT', 'Solde Récup'].map((l, i) => (
              <span
                ...
                style={{
                  fontSize: 11,
                  fontWeight: 700,
                  color: ['#8B4A5A', '#4F7EF7', '#16A34A', '#B45309'][i],
                  textTransform: 'uppercase',
                  letterSpacing: '0.07em',
                  textAlign: 'center',
                }}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {equipe.map((emp) => (
              <LigneSalarie key={emp.id} emp={emp} />
            ))}
          </div>
      ...
      {!isManager && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ...
            {
              label: 'Solde Récup',
              value: soldes?.recup_solde,
              acquis: soldes?.recup_acquis,
              pris: soldes?.recup_pris,
              accent: '#B45309',
              bg: '#FFFBEB',
            },
          ].map((s, i) => (
