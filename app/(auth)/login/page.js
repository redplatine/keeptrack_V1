  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      let { data: emp, error: empError } = await supabase
        .from('employes')
        .select('id, role, prenom, nom, email, user_id, entreprise_id')
        .eq('user_id', user.id)
        .maybeSingle()
  ...
  return (
    ...
    >
      ...
      >
        ...
        <circle cx="90%" cy="-5%" r="260" fill="none" stroke="#4A2330" strokeWidth="2" opacity="0.35" />
        <circle cx="10%" cy="-5%" r="220" fill="none" stroke="#6B2F42" strokeWidth="2" opacity="0.25" />
        <circle cx="50%" cy="2%" r="120" fill="none" stroke="#8B4A5A" strokeWidth="1.5" opacity="0.2" />
      </svg>
                ...
                >
                  <div
                    style={{
                      ...
                      height: '44px',
                      borderRadius: '12px',
                      background: 'linear-gradient(135deg, #3B63D4, #4F7EF7)',
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
                    }}
              ...
              >
                <div
                  style={{
                    ...
                    height: '6px',
                    borderRadius: '50%',
                    background: role === 'admin' ? '#6B2F42' : '#4F7EF7',
                  }}
                />
                <span
                  style={{
                    fontSize: '12px',
                    fontWeight: 600,
                    color: role === 'admin' ? '#6B2F42' : '#3B63D4',
                  }}
                >
