    ...
    if (tokenHash && type) {
      supabase.auth.signOut().then(() => {
        supabase.auth.verifyOtp({ token_hash: tokenHash, type }).then(({ error }) => {
          if (error) setError('Lien invalide ou expiré. Contactez votre administrateur.')
          else setReady(true)
        })
      })
    } else {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center', marginBottom: '36px' }}>
          ...
          <p style={{ fontSize: '13px', color: '#A8A29E', margin: 0 }}>Créez votre mot de passe</p>
        </div>
        ...
        {!ready && !error && (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <p style={{ color: '#A8A29E', fontSize: '13.5px', margin: 0 }}>Vérification du lien…</p>
          </div>
        )}
